...
import {
  ContainerInstance,
  UnleashServerInterface,
} from "./lib/BaseContainers";
...
describe.each(servers)(`$type`, (server) => {
  ...
  describe.each(tests)(`%s suite`, (testName) => {
    ...
    beforeAll(async () => {
      if (!initialized) {
        ...
        if (process.env.DEBUG) {
          const ngrepContainer = await GenericContainer.fromDockerfile(
            path.resolve("./src/tools/ngrep"),
            "Dockerfile"
          ).build();
    ...
    describe.each(sdks)(`$name SDK`, (sdkTestConfig) => {
      ...
      const splitFn = (t: ITestDef) =>
        excludedForSDK.filter(
      ...
      const [skipVariants, evaluateVariants] = partition(
        definition.variantTests || [],
        splitFn
      );
      if (skipVariants.length > 0)
        test.skip.each(skipVariants)(`$description`, () => {});
      if (evaluateVariants.length > 0) {
        test.each(evaluateVariants)(`$description`, async (testCase) => {
          ...
          expect(result).toEqual(
            expect.objectContaining(testCase.expectedResult)
          );
        });
      }
    });
